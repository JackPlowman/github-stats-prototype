...
def catalogue_repository(file_path: str, exclude_paths: list[str] | None = None) -> dict[str, list[str]]:
    """Catalogue the repository.
    ...
    if exclude_paths is None:
        ...
        # Globs not yet supported
    iterator = Path(file_path).walk()
    for root, _dirs, files in iterator:
        if check_for_excluded_dirs(root, exclude_paths):
            continue
        for file in files:
